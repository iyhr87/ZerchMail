...
function selectedRowClass(rowIndex) {
    ...
    if (rowIndex == selectedRowIndex.value) return 'bg-cyan-100 hover:bg-cyan-300';

    return '';
}

function setIndexSelectedRow(i) {
    if (selectedRowIndex.value == i + 1) {
        selectedRowIndex.value = 0;
        return;
    }

    selectedRowIndex.value = i + 1;
}
</script>
    ...
    <div class="sm:w-full m-4">
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg border">
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 table-fixed">
                ...
                <tbody>
                    <tr v-if="searchStatus != 'empty'" v-for="(mail, i ) in mailsTable"
                        @click="$emit('selectRow', i); setIndexSelectedRow(i)"
                        class=" border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 cursor-pointer"
                        :class="selectedRowClass(i + 1)">
                        <td class="px-6 py-4">
                            {{ i+ 1}}
                        </td>
